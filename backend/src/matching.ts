...
export type NegotiationStyle = "Competitive" | "Collaborative" | "Accommodating" | "Avoiding" | "Compromising" | "Analytical" | "Directive" | "Emotive";
export type PerceivedValue = "Innovation" | "Energy" | "Authority" | "Excellence" | "Trust" | "Insights" | "Security";
...
export type MatchResult = {
  agent: AgentProfile;
  score: number;
  label: "Excellent Fit" | "Strong Fit" | "Good Fit" | "Review Carefully";
  reason: string;
...
export const agentArchetypeMap: Record<string, Omit<AgentProfile, "id" | "name">> = {
  ...
  "The Supporter": { archetype: "The Supporter", interactionStyle: "Motivator", focus: "Pragmatic", stressResponse: "Fawn", perceivedValue: "Trust", negotiationStyle: "Accommodating" },
  ...
  "The Producer": { archetype: "The Producer", interactionStyle: "Facilitator", focus: "Pragmatic", stressResponse: "Fight", perceivedValue: "Excellence", negotiationStyle: "Directive" },
...
export const primaryMatchPercentages: Record<string, { clientArchetype: string; percentage: number }> = {
  ...
  "The Supporter": { clientArchetype: "The Advocate", percentage: 93 },
  ...
  "The Producer": { clientArchetype: "The Producer", percentage: 95 },
...
export function calculateAgentClientMatch(client: ClientProfile, agent: AgentProfile): MatchResult {
  const score = Math.round(
    scoreOrientationFit(client, agent) * 0.30 +
    scoreStyleFit(client, agent) * 0.25 +
    scoreStressFit(client, agent) * 0.25 +
    scoreNegotiationFit(client, agent) * 0.10 +
    scoreValueFit(client, agent) * 0.10
  );
  const label = score >= 90 ? "Excellent Fit" : score >= 80 ? "Strong Fit" : score >= 70 ? "Good Fit" : "Review Carefully";
  return {
    agent,
    score,
    label,
    reason: buildMatchReason(client, agent),
    sourceBehavior: client.source === "qr"
...
export function rankAgentsForClient(client: ClientProfile, agents: AgentProfile[]): MatchResult[] {
  return agents.map(agent => calculateAgentClientMatch(client, agent)).sort((a, b) => b.score - a.score);
}

export function buildMatchReason(client: ClientProfile, agent: AgentProfile): string {
  const orientation = client.orientation === "Driver"
  ...
  const style = client.style === "Design-Focused"
